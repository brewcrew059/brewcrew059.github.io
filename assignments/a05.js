			...
			yAxes: [{
				ticks: {
					...
					callback: function(label, index, labels) {
						return (label == 10 || label == 100 || label == 1000 || label == 10000 || label == 100000 || label == 1000000 || label == 10000000) ?
							label / 1000 + 'k' : "";
					}
				},
				scaleLabel: {
...
function loadContent() {
	...
	xhttp.onreadystatechange = function() {
		...
		if (this.readyState == 4 &&
			...
			totalDeathsOver50000 = _.orderBy(totalDeathsOver50000, "TotalDeaths", "desc");

			//for (let i=0; i<totalDeathsOver50000.length; i++){
			//console.log(totalDeathsOver50000[i]);
			//}
			...
			chartData.data.datasets[0].data = totalDeathsOver50000.map(
				(x) => x.TotalConfirmed);
			chartData.data.datasets[1].data = totalDeathsOver50000.map(
				(x) => x.TotalDeaths);
			chartData.data.datasets[2].data = totalDeathsOver50000.map(
				(x) => x.TotalConfirmedPer100000);
			chartData.options.title.text = "Covid 19 Hotspots (" + dayjs().format("YYYY-MM-DD") + ")";
	...
	/**
	localStorage.setItem("time1", null);
	
	if(localStorage.getItem("time1") === null){
	  localStorage.setItem("time1", dayjs());
	  xhttp.open("GET", URL, true);
	  xhttp.send();
	}
	else{
	  myChart = new Chart(localStorage.getItem("ctx"), localStorage.getItem("chartData"));
	}
	**/
